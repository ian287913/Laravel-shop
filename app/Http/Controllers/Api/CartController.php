...
class CartController extends Controller
{
    public function show(){
        $carts = User::all()->where('id', auth('api')->user()->getAuthIdentifier())->first()->carts->sortBy('created_at');
        $data = array();
    ...
    public function store(Request $request){
        $carts = User::all()->where('id', auth('api')->user()->getAuthIdentifier())->first()->carts;
        foreach ($carts as $cart){
